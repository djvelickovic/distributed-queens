 ...
 * <ul>
 * <li>A <code>servent_list.properties</code> file (explained in {@link Config} class</li>
...
public class MultipleServentStarter {
	...
	private static void startServentTest(String testName) {
		...
		Config.readConfig(testName+"/servent_list.properties");
		
		Config.timestampedStandardPrint("Starting multiple servent runner. "
				+ "If servents do not finish on their own, type \"stop\" to finish them");
		
		int serventCount = Config.getServentCount();
		
		for(int i = 0; i < serventCount; i++) {
			try {
				ProcessBuilder builder = new ProcessBuilder("java", "-cp", "bin/", "app.NodeMain",
		...
		Config.timestampedStandardPrint("All servent processes finished. Type \"stop\" to exit.");
	}
	
	public static void main(String[] args) {
		startServentTest("ly_snapshot_repeat");
