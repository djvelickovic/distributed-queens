...
public class RoutingService {
    ...
    private static final Set<String> broadcasts = ConcurrentHashMap.newKeySet();
...
//        logger.error("Unable to find next hop by algorithm. Giving random neighbour.");
    ...
    @Async
    public void broadcast(Integer from, String broadcastId, String method, String payload) {
        // TODO: Validate

        if (!broadcasts.add(broadcastId)) {
            return;
        }

        try {
            logger.info("Broadcasting message {}. Action: {}", payload, method);

            List<Result> broadcastResults = Network.neighbours.values().stream()
